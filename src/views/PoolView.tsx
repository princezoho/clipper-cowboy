...
interface Props {
  ...
  onChanged?: () => void;
  poolDir?: string;
  /** Reloads every tab, for changes that touch more than the pool. */
  onSampleLoaded?: () => void;
}
...
export default function PoolView({
  ...
  if (items.length === 0) {
    return (
      <EmptyGuide
        title="No footage in this project yet"
        lead="The Pool is your intake: every source montage you drop into the project folder shows up here with its clip count, drafts, and saved regions."
        projectDir={poolDir}
        steps={[
          <>
            Put a few video files in the project folder shown below. Finder
            drag-and-drop is fine; subfolders are fine too.
          </>,
          <>
            Come back to this tab. Clipper rescans on focus, or press{" "}
            <span className="font-semibold text-ink-200">Refresh</span> in the
            header.
          </>,
          <>
            Open a source, set in and out points, name the moment, and export.
            Keepers land in <span className="font-semibold text-ink-200">Library</span>.
          </>,
        ]}
        primary={
          <LoadStarterProjectButton
            onLoaded={() => (onSampleLoaded ?? onChanged)?.()}
          />
        }
      >
        <p className="text-xs leading-relaxed text-ink-500">
          The sample project is a handful of synthetic test-pattern clips
          generated by ffmpeg. It is there to show the workflow, and you can
          delete it from Finder at any time.
        </p>
      </EmptyGuide>
    );
  }
